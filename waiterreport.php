...
<div class="container">
    <div class="row"  style="margin-left:300px;">
      <div><h2 style="margin-left:90px;">Get Waiter Wages</h2>  
        <br>   
	...
	<?php
      $GLOBAL['twages'] = 0; 
         ...
         if(isset($_POST['cdate'])):
          ...
          if($w_id != '' && $date_s != '' && $date_e != ''){
           $result= $mysqli->query("SELECT * FROM wages WHERE w_id = '$w_id' AND  datetime BETWEEN '$date_s' AND '$date_e'")or die($mysqli->error);
          }
          elseif ($w_id != '') {
            	$result= $mysqli->query("SELECT * FROM wages WHERE w_id = '$w_id'") or die($mysqli->error);
          }
          elseif ($date_s != '' && $date_e != '') {
            $result= $mysqli->query("SELECT * FROM wages WHERE datetime BETWEEN '$date_s' AND '$date_e'") or die($mysqli->error);
          }
          elseif ($w_id != '' && $date_s != '') {
            $result= $mysqli->query("SELECT * FROM wages WHERE w_id = '$w_id' AND datetime = '$date_s'") or die($mysqli->error);
          }
          elseif ($date_s != '') {
            $result= $mysqli->query("SELECT * FROM wages WHERE datetime = '$date_s'") or die($mysqli->error);
          }
          else {
            $result= $mysqli->query("SELECT * FROM wages WHERE w_id = '$w_id' AND datetime = '$date_s'") or die($mysqli->error);
          }
          $twages=0;
           while ($row = $result->fetch_assoc()):?>
			...
			<?php $GLOBAL['twages'] = $GLOBAL['twages']+   (int)$row['wages'];?>
...
<div class="container">
	<h2>Total Wages :<?php echo $GLOBAL['twages'];?> </h2>
</div>
